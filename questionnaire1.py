...
# INJECT_STAGE_NUMBERS = {0, 1, 3, 5, 7}  # 0=Initial, 1=Inject1, 3=Inject2, 5=Inject3, 7=Inject4
# def is_inject_stage(stage: int) -> bool:
#     """Return True if the dm_stage corresponds to an inject (non‑decision) stage."""
#     return stage in INJECT_STAGE_NUMBERS

# def get_condition_key_for_decisions29to30():
#     """
#     Return the list of decision dicts for decisions 33–34
#     based on FD answers to Decisions 7,13,23.
#     """
#     key = (
#         get_role_decision_answer("Decision 12",  "FD"),
#         get_role_decision_answer("Decision 15", "FD"),
#         get_role_decision_answer("Decision 23", "FD"),
#     )
#     return decisions33to34.get(key, [])

# def get_condition_key_for_decisions31to38():
#     """
#     Return the list for decisions 35–43 based on FD answers to 7,13,23,34.
#     """
#     key = (
#         get_role_decision_answer("Decision 12",  "FD"),
#         get_role_decision_answer("Decision 15", "FD"),
#         get_role_decision_answer("Decision 23", "FD"),
#         get_role_decision_answer("Decision 34", "FD"),
#     )
#     return decisions35to43.get(key, [])



# def process_decisions(decisions: list[dict], on_submit=None):
#     """
#     Rende r a próxima decisão não respondida nesta lista para o role atual.
#     `on_submit(answer_dict)` é chamado com {"inject":..., "answer":...}.
#     Retorna True se havia decisão e foi mostrada; False se nenhuma pendente.
#     """
#     role = st.session_state.get("dm_role")
#     answered_map = st.session_state.get("answers", {})

#     # encontrar próxima não respondida
#     for dec in decisions:
#         inj = dec["inject"]
#         if inj in answered_map:
#             continue

#         # role-specific override
#         qdata = dec
#         rs = dec.get("role_specific", {})
#         if role in rs:
#             qdata = {**dec, **rs[role]}

#         st.subheader(f"{inj}")
#         st.write(qdata.get("text",""))

#         options = qdata.get("options", [])
#         multi   = qdata.get("multi", False)

#         if multi:
#             chosen = st.multiselect("Select one or more:", options, key=f"dec_{inj}")
#         else:
#             placeholder = "-- Select an option --"
#             radio_opts = [placeholder] + options
#             chosen = st.radio("Choose:", radio_opts, key=f"dec_{inj}")
#             if chosen == placeholder:
#                 chosen = None

#         if st.button("Submit ➡", key=f"submit_{inj}", disabled=(multi and not chosen) or (not multi and chosen is None)):
#             final_answer = chosen if multi else chosen
#             # persist local
#             st.session_state.answers[inj] = final_answer
#             if callable(on_submit):
#                 on_submit({"inject": inj, "answer": final_answer})
#             st.rerun()
#         return True  # mostrou algo
#     return False
# 
# 
# 
# 
# 
# # def get_role_decision_answer(inject_prefix: str, role: str) -> str | None:
#     """Fetch the answer_text for the given inject_prefix by the given role."""
#     sim_id = st.session_state.get("simulation_id")
#     if sim_id is None:
#         return None

#     # 1) Find the participant.id for that role
#     try:
#         part_res = (
#             supabase
#             .from_("participant")
...
#         )
#     except Exception as e:
#         st.warning(f"Could not look up {role} in this simulation: {e}")
#         return None

#     if not getattr(part_res, "data", None):
#         st.warning(f"No participant row found for role '{role}'.")
#         return None

#     part_id = part_res.data["id"]

#     # 2) Now fetch the one answer where inject LIKE 'prefix%'
#     try:
#         ans_res = (
#             supabase
#             .from_("answers")
...
#             .eq("id_simulation", sim_id)
#             .eq("id_participant", part_id)
#             .like("inject", f"{inject_prefix}%")
#             .single()
#             .execute()
#         )
#     except Exception:
#         # either network error or no rows → treat as “not answered yet”
#         return None

#     if not getattr(ans_res, "data", None):
#         return None

#     return ans_res.data["answer_text"]
# 
# 
# 
# 
...
# 
# # def run(supabase, simulation_name: str, role: str):
#     # 1) initialize our private bits
#     #_normalize = {
#      #   "Commander (CMO,IV2)": "Commander (CMO, IV1)",
#     #}
#     #role = _normalize.get(role, role)
#     st.session_state.dm_role = role
#     initial_stage() 
#     st.session_state.setdefault("tlx_answers", {})
#     st.session_state.setdefault("test_mode", False)
#     st.session_state.setdefault('all_questions', None)
#     st.session_state.setdefault("loaded_14to23", False)
#     st.session_state.setdefault("loaded_33to34", False)
#     st.session_state.setdefault("loaded_29to32", False)
#     st.session_state.setdefault("loaded_35to43", False)
#     st.session_state.simulation_name = simulation_name
#     apply_vital_consequences(st.session_state.get("answers", {}))
#     sim_id  = st.session_state.get("simulation_id")
#     part_id = st.session_state.get("participant_id")
#     if sim_id is None or part_id is None:
#         st.error("❌ Simulation or participant ID missing!")
#         return
#     answers = st.session_state.get("answers", {})
#     answer_times = st.session_state.get("answer_times", {})

#     # st.write("🔍 DEBUG stage:", st.session_state.dm_stage)
#     # st.write("🔍 DEBUG idx:", st.session_state.current_decision_index)
#     # st.write("🔍 DEBUG #qs:", len(st.session_state.all_questions or [])) 

#     st.session_state.setdefault("answer_times", {})
#     st.session_state.setdefault("all_questions", [])
#     if not st.session_state.all_questions:
#         st.session_state.all_questions = decisions1to15[:] 

#     # inside your Next-click handler, before you stash the answer
#     st.session_state.question_text_map = {}
#     for d in st.session_state.all_questions:
#         inject = d["inject"]
#         if "text" in d:
#             # generic question
#             txt = d["text"]
#         else:
#             # role-specific question, but fall back to empty if missing
#             rs = d.get("role_specific", {})
#             txt = rs.get(st.session_state.dm_role, {}).get("text", "")
#         st.session_state.question_text_map[inject] = txt
...
#         with col1:
#             st.write("Are you ready to embark in this mission?")
#         with col2:
#             if st.button("▶️ Start Simulation"):
#                 payload = {
#                     "id_simulation":  sim_id,
#                     "id_participant": part_id,
#                     "inject":         "Initial Situation",
#                     "question_text":  "",
#                     "answer_text":    "DONE",
#                 }
#                 try:
...
#                     supabase.from_("participant") \
#                             .update({
#                                 "current_inject": "Initial Situation",
#                                 "current_answer": "DONE",
#                             }) \
#                             .eq("id", part_id) \
#                             .execute()
#                 except Exception as e:
#                     st.error(f"❌ Could not record Inject 1 or mirror state: {e}")
#                     return
...
#     elif stage == 1:
#         inject1()
#             # ← only proceed to the group‐tally *after* this user has clicked Next
#         if not st.session_state.inject1_clicked:
#             return
...
#             ).data or []
#         except Exception as e:
#             st.error(f"❌ Could not fetch ready count: {e}")
#             return

#         # compute unique‐participant count
#         unique_ids = {r["id_participant"] for r in done_rows}
#         ready      = len(unique_ids)

#         # our master list of 8 roles
#         expected_roles = [
...
#         # 3) fetch which roles have done it
#         try:
#             list_res = (
#                 supabase
#                 .from_("answers")
...
#             )
#             done_roles = [
#                 r["participant"]["participant_role"]
#                 for r in (list_res.data or [])
#             ]
#         except Exception as e:
#             st.error(f"❌ Could not fetch done roles: {e}")
#             return

#         # 4) wait‐for‐all logic
#         if not st.session_state.get("test_mode", False) and ready < len(expected_roles):
#             st.warning(f"Waiting for all roles to advance past Inject 1… ({ready}/{len(expected_roles)})")
#             #st.write("✅ Done:", done_roles)
#             #st.write("⏳ Still waiting on:",[r for r in expected_roles if r not in done_roles])
#             if st.button("🔄 Refresh status", key="refresh_inject1"):
#                 st.rerun()
#             return

#         # 5) everyone’s in — show “Next”
#         if st.button("Next ➡", key="after_inject1"):
#             payload = {
#                 "id_simulation":  sim_id,
#                 "id_participant": part_id,
#                 "inject":         "Inject 1",
...
#                 st.error(f"❌ Could not record Inject 1 or mirror state: {e}")
#                 return
#             apply_vital_consequences(st.session_state.answers)
#             st.session_state.dm_stage = 2
#             st.session_state.current_decision_index = 1
#             st.rerun()
    
#     elif stage == 2:
#         if st.session_state.current_decision_index > len(st.session_state.all_questions):
#             st.session_state.dm_stage = 3
#             st.rerun()
#             return
#         display_current_decision()
#         return
...
#         display_inject2()
#         if not st.session_state.inject2_clicked:
#             if st.button("Next ➡", key="inject2_next"):
#                 payload = {
#                     "id_simulation":  sim_id,
#                     "id_participant": part_id,
#                     "inject":         "Inject 2",
...
#                     supabase.from_("participant") \
#                     .update({
#                         "current_inject": payload["inject"],
#                         "current_answer": "DONE",
#                     }) \
#                     .eq("id", part_id) \
#                     .execute()
#                 except Exception as e:
#                     st.error(f"❌ Could not record Inject 2: {e}")
#                     return

#                 st.session_state.inject2_clicked = True
#                 st.rerun()
#             return
...
#         if not getattr(done_rows, "data", None):
#             st.error("❌ Could not fetch ready count.")
#             return

#         # now compute the unique‐participant count in Python
#         unique_ids = {row["id_participant"] for row in done_rows.data}
#         ready = len(unique_ids)
#         done_roles = [
#             "FE-3 (EVA2)", "Commander (CMO,IV2)", "FE-1 (EVA1)",
#             "FE-2 (IV1)",   "FD",                   "FS",
...
#         # 3) list which roles are DONE (via the FK to participant)
#         try:
#             done_rows = (
#                 supabase
#                 .from_("answers")
...
#         # 4) wait‐for‐all logic
#         if not st.session_state.get("test_mode", False) and ready < len(roles):
#             st.warning(f"Waiting for all roles to advance past Inject 2… ({ready}/{len(roles)})")
#             #st.write("✅ Done:", done_roles)
#             #st.write("⏳ Still waiting on:", [r for r in roles if r not in done_roles])
#             if st.button("Refresh status", key="refresh_inject2"):
#                 st.rerun()
#             return

#         # 5) once everyone’s ready, proceed…
#         cond = (
#         get_role_decision_answer("Decision 12", "FD"), 
#         get_role_decision_answer("Decision 15", "FD")
#         )
#         # decisions14to23 is a dict mapping (ans7, ans13) → list of questions :contentReference[oaicite:2]{index=2}
#         st.session_state.all_questions        = decisions14to23.get(cond, [])
#         st.session_state.current_decision_index = 1
#         st.session_state.loaded_14to23         = True
#         st.session_state.dm_stage               = 4
#         st.rerun()

    

#     elif stage == 4:
#         # only advance to Inject 3 once you've shown *all* follow-ups
#         # q = st.session_state.all_questions[st.session_state.current_decision_index-1]
#         # st.write("→ I’m about to show:", q["inject"])
#         if st.session_state.current_decision_index is None or st.session_state.current_decision_index > len(st.session_state.all_questions):
#             st.session_state.dm_stage = 5
#             st.rerun()
#             return
#         display_current_decision()
#         return
...
#         # show the Inject 3 prompt/UI
#         inject3()

#         # 1) record this participant’s “DONE” click
#         if not st.session_state.inject3_clicked:
#             if st.button("Next ➡", key="inject3_next"):
#                 payload = {
#                     "id_simulation":  sim_id,
#                     "id_participant": part_id,
#                     "inject":         "Inject 3",
...
#                     .execute()
#                 except Exception as e:
#                     st.error(f"❌ Could not record Inject 3: {e}")
#                     return

#                 st.session_state.inject3_clicked = True
#                 st.rerun()
#             return

#         # 2) count how many distinct participants have clicked DONE
#         try:
#             done_rows = (
#                 supabase
#                 .from_("answers")
...
#         # 3) list which roles are DONE (joining via the FK to participant)
#         try:
#             done_rows = (
#                 supabase
#                 .from_("answers")
...
#         # 4) wait‐for‐all logic
#         if not st.session_state.get("test_mode", False) and ready < len(roles):
#             st.warning(f"Waiting for all roles to advance past Inject 3… ({ready}/{len(roles)})")
#             #st.write("✅ Done:", done_roles)
#             #st.write("⏳ Still waiting on:", [r for r in roles if r not in done_roles])
#             if st.button("Refresh status", key="refresh_inject3"):
#                 st.rerun()
#             return
...
#     elif stage == 6:
#         # once we’ve exhausted 24–28, move on to Inject 4
#         if st.session_state.current_decision_index > len(st.session_state.all_questions):
#             st.session_state.dm_stage = 7
#             st.rerun()
#             return
...
#         # 1) show the Inject 4 UI
#         inject4()

#         # 2) on first click, record DONE in Supabase
#         if not st.session_state.inject4_clicked:
#             if st.button("Next ➡", key="inject4_next"):
#                 payload = {
#                     "id_simulation":  sim_id,
#                     "id_participant": part_id,
#                     "inject":         "Inject 4",
...
#                     .execute()
#                 except Exception as e:
#                     st.error(f"❌ Could not record Inject 4: {e}")
#                     return

#                 st.session_state.inject4_clicked = True
#                 st.rerun()
#             return

#         # 3) count how many distinct participants have clicked DONE
#         try:
#             done_rows = (
#                 supabase
#                 .from_("answers")
...
#         # 4) list which roles are DONE (via the FK to participant)
#         try:
#             done_rows = (
#                 supabase
#                 .from_("answers")
...
#         # 5) wait-for-all logic
#         if not st.session_state.get("test_mode", False) and ready < len(roles):
#             st.warning(f"Waiting for all roles to advance past Inject 4… ({ready}/{len(roles)})")
#             #st.write("✅ Done:", done_roles)
#             #st.write("⏳ Still waiting on:", [r for r in roles if r not in done_roles])
#             if st.button("Refresh status", key="refresh_inject4"):
#                 st.rerun()
#             return
...
#             st.session_state.loaded_33to34 = False
#             st.rerun()
#         return

#     if "loaded_33to34" not in st.session_state:
#         st.session_state.loaded_33to34 = False

#     elif stage == 8:
#         sim_id  = st.session_state.simulation_id
#         part_id = st.session_state.participant_id
#         expected_roles = 8  # or len(your_roles_list)

#         # 1) Count how many distinct participants have answered Decision 32
#         try:
#             ans32_count = (
#                 supabase
#                 .from_("answers")
#                 .select("id_participant", count="exact")
#                 .eq("id_simulation", sim_id)
#                 .like("inject", "Decision 32%")      # matches "Decision 32 (hh:mm:ss)"
#                 .neq("answer_text", "SKIP")
#                 .execute()
#                 .count or 0
#             )
#         except Exception as e:
#             st.error(f"❌ Could not fetch Decision 32 tally: {e}")
#             return

#         # 2) If not everybody’s in, show a waiting panel
#         if ans32_count < expected_roles:
#             st.warning(f"Waiting for all roles to complete Decision 32… ({ans32_count}/{expected_roles})")
#             if st.button("🔄 Refresh Decision 32 status", key="refresh_32"):
#                 st.rerun()
#             return

#         # 3) Everyone’s answered Decision 32 → advance to stage 9
#         #    and load the correct 33–34 block exactly once
#         cond = (
#             get_role_decision_answer("Decision 12",  "FD"),
#             get_role_decision_answer("Decision 15", "FD"),
#             get_role_decision_answer("Decision 23", "FD"),
#         )
#         questions = decisions33to34.get(cond, [])
#         if not questions:
#             st.error(f"No follow-up found for Decision 33–34 with key {cond!r}")
#             return

#         st.session_state.all_questions          = questions
#         st.session_state.current_decision_index = 1
#         st.session_state.loaded_33to34          = True
#         st.session_state.dm_stage               = 9
#         st.rerun()

#     elif stage == 9:
#         # **first** time in stage 9: load the right 33–34 block
#         if not st.session_state.loaded_33to34:
#             cond = (
#                 get_role_decision_answer("Decision 12",  "FD"),
#                 get_role_decision_answer("Decision 15", "FD"),
#                 get_role_decision_answer("Decision 23", "FD"),
...
#             questions = decisions33to34.get(cond)
#             if not questions:
#                 st.error(f"No 33–34 block for scenario {cond!r}")
#                 return
#             st.session_state.all_questions          = questions
#             st.session_state.current_decision_index = 1
#             st.session_state.loaded_33to34          = True

#         # now show Decision 33 or 34, or advance to stage 10
#         if st.session_state.current_decision_index <= len(st.session_state.all_questions):
#             display_current_decision()
#         else:
#             st.session_state.dm_stage = 10
#         return


#     elif stage == 10:
#         qs = st.session_state.all_questions or []
#         # 1) First time finishing 33–34, load 35–43 exactly once
#         if qs and st.session_state.current_decision_index > len(qs) \
#         and not st.session_state.loaded_35to43:
...
#                 get_role_decision_answer("Decision 34", "FD"),
#             )
#             st.session_state.all_questions            = decisions35to43.get(cond34, [])
#             st.session_state.current_decision_index   = 1
#             st.session_state.loaded_35to43            = True
#             st.rerun()
#             return

#         # 2) Once 35–43 are themselves exhausted, move to Thank You
#         if st.session_state.loaded_35to43 \
#         and st.session_state.current_decision_index > len(st.session_state.all_questions):

#             st.session_state.dm_stage = 11
#             st.rerun()
#             return

#         # 3) Otherwise just display the next decision
#         display_current_decision()
#         return
    
#     # 4) After all decisions, show a “thank you” + TLX prompt
#     elif stage == 11:
#         st.header("🎉 Thank you for completing the simulation!")
...
#     elif stage == 13:
#         st.success("🎉 Thank you for completing the TLX!")
#         st.session_state.dm_finished = True        
#         return
# 
# 
# 
...
#     expected_roles = ["FE-3 (EVA2)","Commander (CMO,IV2)","FE-1 (EVA1)","FE-2 (IV1)","FD","FS","BME","CAPCOM"]
#     sim_id  = st.session_state.simulation_id    # bigserial PK from public.simulation
#     part_id = st.session_state.participant_id   # bigserial PK from public.participant
#     raw_score = get_correct_answer(inject)
#     score = raw_score if isinstance(raw_score, (int, float)) else 0

#     if "role_specific" in decision and "text" not in decision and role not in decision["role_specific"]:
#         payload = {
#             "id_simulation":  sim_id,
#             "id_participant": part_id,
#             "inject":         inject,
...
#                 .execute()
#         except Exception as e:
#             st.error(f"❌ Could not save or mirror SKIP: {e}")
#         st.session_state.current_decision_index += 1
#         st.rerun()
#         return
        
#     if "role_specific" in decision and role in decision["role_specific"]:
#         qdata = decision["role_specific"][role]
#     else:
#         qdata = decision

#     qdata = decision.get("role_specific", {}).get(role, decision)

#     if any(inject.startswith(k) for k in key_decisions):
#         if role == "FD":
#             # 1a) FD’s turn to answer:
#             st.subheader(f"{inject}  {decision['text']}")

#             if "decision_start_time" not in st.session_state:
#                 st.session_state.decision_start_time = time.time()
#             max_time = decision.get("max_time", 300)
#             unique_timer_id = f"timer_fd_{i}"
#             timer_html = f'''
# <div id="{unique_timer_id}" style="font-family: 'Georgia', serif;
#      font-size: 24px; font-weight: bold; color: red;
#      background-color: #F4F6F7; padding: 10px;
...
#     }} else {{
#       var extra = -timeleft;
#       var penalty = 1 + 2 * Math.floor(extra/10);
#       timer.innerHTML = "Time’s up! Penalty: " + penalty + " pts";
#     }}
#     timeleft--;
...
#                 key=f"dec_{inject}"
#             )
#             if st.button("Submit ➡", key=f"fd_submit_{inject}"):

#                 elapsed = int(time.time() - st.session_state.decision_start_time)
#                 hrs, rem = divmod(elapsed, 3600)
#                 mins, secs = divmod(rem, 60)
#                 response_time = f"{hrs:02d}:{mins:02d}:{secs:02d}"
#                 # compute your elapsed, score, etc.
...
#                     "systems_procedural_knowledge": 0,
#                 }
#                 if "scores" in qdata and answer in qdata["options"]:
#                     idx = qdata["options"].index(answer)
#                     sco = qdata["scores"][idx]
#                     cat_payload.update({
#                         "basic_life_support":           sco.get("Basic_Life_Support",           0),
#                         "primary_survey":               sco.get("Primary_Survey",               0),
#                         "secondary_survey":             sco.get("Secondary_Survey",             0),
#                         "definitive_care":              sco.get("Definitive_Care",              0),
#                         "crew_roles_communication":     sco.get("Crew_Roles_Communication",     0),
#                         "systems_procedural_knowledge": sco.get("Systems_Procedural_Knowledge", 0),
#                     })
...
#                             .execute()
#                 except Exception as e:
#                     st.error(f"❌ Could not save your answer or state: {e}")
#                     return
...
#             return
#         else:
#             # 1b) Everyone else just waits on FD’s answer:
#             st.subheader(f"{inject}  {decision['text']}")
#             # fetch FD’s row
#             part_q = (
#                 supabase
...
#             rows = part_q.data or []
#             if not rows:
#                 st.warning("⚠️ Waiting for the Flight Director to join the simulation…")
#                 return
#             fd_id = rows[0]["id"]

#             # 2) Fetch FD’s answer for this inject
#             ans_q = (
#                 supabase
#                 .from_("answers")
#                 .select("answer_text")
...
#                 .execute()
#             )
#             ans_rows = ans_q.data or []
#             if not ans_rows:
#                 st.warning("⏳ Waiting for the Flight Director to make this decision…")
#                 if st.button("🔄 Refresh FD’s answer"):
#                     st.rerun()
#                 st.stop()

#             fd_ans = ans_rows[0]["answer_text"]
#             st.info(f"Flight Director chose: **{fd_ans}**")
#             st.session_state.answers[inject] = fd_ans

#             # 3) Now show a single “Next ➡” for everyone else
#             if st.button("Next ➡", key=f"fd_next_{i}"):
#                 st.session_state.current_decision_index += 1
#                 st.rerun()
#             return


#     if inject in st.session_state.answers:
#         st.subheader(f"{inject}  {qdata.get('text','')}")
#         user_ans = st.session_state.answers.get(inject)
#         if user_ans is not None:
#             st.markdown(f"**Your answer:** {user_ans}")
#         # fetch how many have done it
#         is_inject = inject.startswith("Inject")

#         # ————————————————————————————————
#         # 1) Fetch all matching answer rows
#         # ————————————————————————————————
#         try:
#             all_rows = (
#                 supabase
...
#             )
#         except Exception as e:
#             st.error(f"❌ Could not fetch response rows for {inject}: {e}")
#             return

#         # ————————————————————————————————
#         # 2) Filter & dedupe in Python
#         # ————————————————————————————————
#         # For an Inject we only count rows with answer_text=="DONE";
#         # for a Decision we ignore the SKIP rows.
#         filtered = []
#         for r in all_rows:
#             txt = r["answer_text"]
#             if is_inject and txt == "DONE":
#                 filtered.append(r)
#             elif not is_inject and txt != "SKIP":
#                 filtered.append(r)

#         # Unique participants who have responded
#         unique_ids = {r["id_participant"] for r in filtered}
#         ready      = len(unique_ids)

#         # Which roles those rows belong to:
#         done_roles = []
#         for r in filtered:
#             role_done = r.get("participant", {}).get("participant_role")
#             if role_done and role_done not in done_roles:
#                 done_roles.append(role_done)

#         # ————————————————————————————————
#         # 3) Wait-for-all logic
#         # ————————————————————————————————
#         if not st.session_state.get("test_mode", False) and ready < len(expected_roles):
#             st.warning(f"Waiting for everyone to finish {inject}… ({ready}/{len(expected_roles)})")
#             #st.write("✅ Done:", done_roles)
#             #st.write("⏳ Still waiting on:", [r for r in expected_roles if r not in done_roles])
#             if st.button("🔄 Refresh", key=f"refresh_{inject}"):
#                 st.rerun()
#             return

#         # ————————————————————————————————
#         # 4) Everybody’s in → one Next button
#         # ————————————————————————————————
#         if st.button("Next ➡", key=f"after_{i}"):
#             apply_vital_consequences(st.session_state.answers)
#             st.session_state.current_decision_index += 1
#             st.rerun()
#         return
...
#     # ——— 3. Multi-role block ———
#     # 2) multi-role branch
#     unique_key = f"ans_{i}_{role}"

#     if "options" in qdata:
#         if qdata.get("multi", False):
#             answer = st.multiselect("", qdata["options"], key=unique_key)
#             if len(answer) > 5:
...
#             answer = st.radio("", radio_options, key=unique_key)
#             if answer == placeholder:
#                 st.error("Please select an option before continuing.")
#                 st.stop()
#     else:
#         st.info("No response required for this message.")
#         answer = qdata.get("text", "")

#     if st.button("Submit ➡", key=f"submit_{i}_{role}"):
...
#         }
#         if "scores" in qdata:
#             scores_list = qdata["scores"]
#             if qdata.get("multi", False):
#                 for choice in answer:  # answer is a list of option‑strings
#                     idx = qdata["options"].index(choice)
#                     sco = scores_list[idx]
#                     cat_payload["basic_life_support"]           += sco.get("Basic_Life_Support",           0)
#                     cat_payload["primary_survey"]               += sco.get("Primary_Survey",               0)
...
#     """
#     Walk through every answered inject in session_state['answers'],
#     ask get_score_breakdown for that answer, and accumulate:
#       - per‐category totals
#       - two domain totals:
#          * medical_knowledge_total  = Basic_Life_Support + Primary_Survey + Secondary_Survey + Definitive_Care
#          * procedural_knowledge_total = Crew_Roles_Communication + Systems_Procedural_Knowledge
#     Returns a dict with all six category sums plus the two domain sums.
#     """
#     cats = {
#         "Basic_Life_Support": 0.0,
#         "Primary_Survey": 0.0,
#         "Secondary_Survey": 0.0,
#         "Definitive_Care": 0.0,
#         "Crew_Roles_Communication": 0.0,
#         "Systems_Procedural_Knowledge": 0.0,
#     }

#     for inject, answer in st.session_state.get("answers", {}).items():
#         bd = get_score_breakdown(inject, answer)
#         # bd has keys matching cats plus "total_score"
#         for c in cats:
#             cats[c] += bd.get(c, 0.0)

#     # now compute domain‐level sums
#     med_total = (
#         cats["Basic_Life_Support"]
#       + cats["Primary_Survey"]
#       + cats["Secondary_Survey"]
#       + cats["Definitive_Care"]
#     )
#     proc_total = (
#         cats["Crew_Roles_Communication"]
#       + cats["Systems_Procedural_Knowledge"]
#     )

#     # stash them if you want
#     st.session_state["dm_totals"] = {
#         **cats,
#         "medical_knowledge_total": med_total,
#         "procedural_knowledge_total": proc_total,
#     }
#     return st.session_state["dm_totals"]

# # in questionnaire1.py

# def compute_team_breakdown(metrics: dict[str, float]) -> dict[str, float]:
#     """
#     Turn teamwork.metrics into the full set of columns we write out:
#       - total_score
#       - Basic_Life_Support_team, Primary_Survey_team, ... (zeros here, unless you actually scored those)
#       - mental_team, physical_team, ... (if you captured TLX on the team side)
#       - leadership_score, teamwork_score, task_management_score
#     """
#     # 1) grab the three domain scores (and overall total)
#     leadership = metrics.get("leadership", 0.0)
#     teamwork   = metrics.get("teamwork",   0.0)
#     task       = metrics.get("task",       0.0)
#     total      = metrics.get("total",      0.0)

#     # 2) optional: TLX‐style ratings, if you have them
#     mental      = metrics.get("Mental Demand",    None)
#     physical    = metrics.get("Physical Demand",  None)
#     temporal    = metrics.get("Temporal Demand",  None)
#     performance = metrics.get("Performance",      None)
#     effort      = metrics.get("Effort",           None)
#     frustration = metrics.get("Frustration",      None)

#     # 3) package everything into the exact keys your INSERT expects:
#     return {
#         # overall
#         "total_score": total,

#         # if you still want the old CHIRON categories on the team side, zero them here:
#         "Basic_Life_Support_team":           0.0,
#         "Primary_Survey_team":               0.0,
#         "Secondary_Survey_team":             0.0,
#         "Definitive_Care_team":              0.0,
#         "medical_knowledge_team":            0.0,
#         "Crew_Roles_Communication_team":     0.0,
#         "Systems_Procedural_Knowledge_team": 0.0,
#         "procedural_knowledge_team":         0.0,

#         # TLX face‐off (if you collected them)
#         "mental_team":       mental,
#         "physical_team":     physical,
#         "temporal_team":     temporal,
#         "performance_team":  performance,
#         "effort_team":       effort,
#         "frustration_team":  frustration,
#         # if you want to compute average:
#         "taskload_team":     (None if None in (mental,physical,temporal,performance,effort,frustration)
#                               else (mental+physical+temporal+performance+effort+frustration)/6),

#         # the three TEAM domains
#         "leadership_score":        leadership,
#         "teamwork_score":          teamwork,
#         "task_management_score":   task,
#     }


# def make_decision(inject, text, options, scores=None, multi=False, role_specific=None):
#     d = dict(base_decision)
#     d.update({
#         'inject': inject,
#         'text': text,
#         'options': options,
#         'scores': scores or [{'medical':0,'procedural':0} for _ in options],
#     })
#     if multi:
#         d['multi'] = True
#     if role_specific:
#         d['role_specific'] = role_specific
#     return d

# def score(medical: int, procedural: int) -> dict[str, int]:
#     return {"medical": medical, "procedural": procedural}


# 4) Score breakdown function
# def get_score_breakdown(inject: str, answer: str) -> dict[str, float]:
#     """
#     Find the decision definition for `inject`, find which option index `answer`
#     corresponds to, and return that option’s score‐dict plus a total.
#     """
...
#     # helper to make a zero‐filled dict
#     def zero():
#         return {
#             "total_score": 0.0,
#             **{cat.replace(" ", "_"): 0.0 for cat in base_cats}
#         }
    
#     target_inject = inject.strip().lower()
#     target_answer = answer.strip().lower()
#     role = st.session_state.get("dm_role", "")

#     # search every block
#     for block in (
#         decisions1to15,
#         *decisions14to23.values(),
#         *decisions24to28.values(),
#         *decisions29to32.values(),
#         *decisions33to34.values(),
#         *decisions35to43.values(),
#     ):
#         for d in block:
#             if d["inject"].strip().lower() != target_inject:
#                 continue

#             # 1) pull out the right options+scores for this decision
#             raw_opts, scores = [], []
#             entry = None

#             if "role_specific" in d:
#                 # try current role first
#                 entry = d["role_specific"].get(role)

#                 # fallback: if they answered an FD‐only decision,
#                 # grab whichever role_specific block actually *has* that option
#                 if entry is None:
#                     for _, val in d["role_specific"].items():
#                         opts = [o.strip().lower() for o in val.get("options", [])]
#                         if target_answer in opts:
#                             entry = val
#                             break
...
#             # if nothing found in role_specific, fall back to top‐level
#             if not entry:
#                 raw_opts = d.get("options", [])
#                 scores   = d.get("scores",  [])

#             opts = [o.strip().lower() for o in raw_opts]

#             # 2) handle multi‐select
#             if d.get("multi", False):
#                 # split on commas, semicolons, slashes, ampersands...
#                 parts = re.split(r'\s*[;,/&]\s*', answer.strip().lower())
#                 out = zero()
#                 for part in parts:
#                     if part in opts:
#                         idx = opts.index(part)
#                         cat_vals = scores[idx]
#                         total = sum(cat_vals.values())
#                         out = {}
...
#             # 3) single‐select: must find exactly one
#             if not opts or not scores or target_answer not in opts:
#                 return zero()

#             idx = opts.index(target_answer)
#             cat_vals = scores[idx]
#             total    = sum(cat_vals.values())

#             out = {"total_score": total}
#             for cat, val in cat_vals.items():
#                 out[cat.replace(" ", "_")] = val
#             return out
